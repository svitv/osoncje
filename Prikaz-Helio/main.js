...
// Izračuna ponovno fizikalne podatke
function setXY() {
...
function draw() {
    ...
    drawImageRotated(context, sun, 26 * sunState.x + canvas.width / 2 - sun.width / 2, 26 * sunState.y + canvas.height / 2 - sun.height / 2, sun.width, sun.height, sunRotation);
    drawImageRotated(context, mer, 26 * mercuryState.x + canvas.width / 2 - mer.width/2, 26* mercuryState.y + canvas.height / 2 - mer.height / 2, mer.width, mer.height, merRotation);
    drawImageRotated(context, v, 26 * venusState.x + canvas.width / 2 - v.width / 2, 26 * venusState.y + canvas.height / 2 - mer.height/2, v.width, v.height, vRotation);
    drawImageRotated(context, e, 26 * earthState.x + canvas.width / 2 - e.width / 2, 26 * earthState.y + canvas.height / 2 - e.height / 2, e.width, e.height, eRotation);
    drawImageRotated(context, mar, 26 * marsState.x + canvas.width / 2 - mar.width/2, 26 * marsState.y + canvas.height / 2 - mar.height/2, mar.width, mar.height, marRotation);
    drawImageRotated(context, j, 26 * jupiterState.x + canvas.width / 2 - j.width/2, 26 * jupiterState.y + canvas.height / 2 - j.height/2, j.width, j.height, jRotation);
    drawImageRotated(context, sat, 26 * saturnState.x + canvas.width / 2 - sat.width/2, 26 * saturnState.y + canvas.height / 2 - sat.height/2, sat.width, sat.height, satRotation);
    drawImageRotated(context, u, 26 * uranusState.x + canvas.width / 2 - u.width/2, 26 * uranusState.y + canvas.height / 2 - u.height/2, u.width, u.height, uRotation);
    drawImageRotated(context, n, 26 * neptuneState.x + canvas.width / 2 - n.width/2, 26 * neptuneState.y + canvas.height / 2 - n.height/2, n.width, n.height, nRotation);
}
